            ...
            <div className="mainAdmin-container">
                ...
                <div className="mini-contianer">
                    <div className="status-excel-container">
                        ...
                        {statuses
                            .filter(status => status.statusText !== "Получено" && role === 'filial' && status.statusText !== "Готов к выдаче")
                            .map(status => (
                        ...
                        {role === 'filial' && filial && filial.filialText && statuses.map((status) => {
                            if (status.statusText === "Готов к выдаче") {
                                return (
                                    <div key={status._id} className="status-el status-pop" onClick={() => handleStatusClick(status._id, `${status.statusText} - ${filial.filialText}`)}>
                                        <p>{status.statusText} - {filial.filialText}</p>
                                    </div>
                                );
                            }
                            return null;
